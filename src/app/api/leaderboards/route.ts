...
export async function GET(request: NextRequest) {
  try {
    ...
    leaderboards.overall = users.map((user, index) => ({
      ...
      rank: index + 1,
      score: Number(user.level) * 1000 + Number(user.xp) + Number(user.coins),
      level: Number(user.level),
      xp: Number(user.xp),
      coins: Number(user.coins),
      achievements: Math.floor(Number(user.level) / 5), // Mock achievements
      stats: {
    ...
    leaderboards.xp = users
      .map((user, index) => ({
        ...
        displayName: user.displayName,
        rank: index + 1,
        score: Number(user.xp),
        level: Number(user.level),
        xp: Number(user.xp),
        coins: Number(user.coins),
        achievements: Math.floor(Number(user.level) / 5),
        stats: {
